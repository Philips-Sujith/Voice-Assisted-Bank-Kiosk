...
#
TRANSITIONS = [
    # idle / awaiting_confirmation / awaiting_auth → intent_received (allows updated/re-prompted intent)
    {"trigger": "voice_intent_received", "source": ["idle", "awaiting_confirmation", "awaiting_auth"], "dest": "intent_received"},
...
class KioskMachine:
    ...
    def on_enter_intent_received(self) -> None:
        ...
        if self.context.backend_auth_needed:
            # Check if customer already authenticated upfront
            if self.context.auth_status == "pass" and self.context.customer_id:
                logger.info(
                    "[%s] intent='%s' requires auth, but customer '%s' is ALREADY authenticated upfront → awaiting_confirmation",
                    self.context.session_id,
                    self.context.intent,
                    self.context.customer_id,
                )
                self.no_auth_needed()    # already authenticated; advance to confirmation
            else:
                logger.info(
