...
- Customer Kiosk (session management & transaction requests)
- Voice Service (vernacular parsing & intent extraction)
- Banking API (orchestrator & FSM state machine)
- Identity Service (face authentication & multi-sample enrollment)
- Security Service (HMAC-SHA256 token service & QR generation)
- Teller Portal (queue & WebSocket events)
...
def test_09_teller_portal_realtime_websocket_event():
    """Test 9: Connect to WebSocket dashboard and verify instant receipt of real-time queue event."""
    async def _run():
        print("\n--- Running Test 9: Teller Portal Real-Time WebSocket Notification ---")
        session_id = str(uuid.uuid4())
        ...
        async with websockets.connect(WS_BACKEND_URL) as ws:
            ...
            assert received_event is not None, "Teller Portal WebSocket did NOT receive new_queue_entry event!"
            ...
            print(f"[PASS] Teller Portal WebSocket received event: Customer='{received_event['customer_display_name']}', Txn='{received_event['transaction_type']}', Amount=₹{received_event['amount']:,}, Token='{received_event['token_id']}'")
