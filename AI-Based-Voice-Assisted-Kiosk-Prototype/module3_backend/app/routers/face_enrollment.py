...
# URL of Module 4 Face Auth service
...
def get_demo_fixtures_dir() -> Path:
    """Resolve demo_fixtures directory from various candidate root paths."""
    candidates = [
        Path.cwd() / "demo_fixtures",
        Path(__file__).resolve().parents[3] / "demo_fixtures",
        Path(__file__).resolve().parents[4] / "demo_fixtures",
        Path("Y:/Bank2/demo_fixtures"),
        Path("Y:/Bank/demo_fixtures"),
    ]
    for p in candidates:
...
def _local_detect_face_fallback(image_b64: str) -> dict:
    ...
    try:
        ...
        yunet_candidates = [
            os.path.join(base_dir, "module4_face_auth", "models", "face_detection_yunet_2023mar.onnx"),
            os.path.join(base_dir, "..", "module4_face_auth", "models", "face_detection_yunet_2023mar.onnx"),
            os.path.join(base_dir, "AI-Based-Voice-Assisted-Kiosk-Prototype", "module4_face_auth", "models", "face_detection_yunet_2023mar.onnx"),
        ]
        yunet_path = next((p for p in yunet_candidates if os.path.exists(p)), None)
        if yunet_path:
            h, w = frame.shape[:2]
