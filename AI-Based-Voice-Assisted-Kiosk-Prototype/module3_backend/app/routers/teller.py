...
@router.get("/teller/queue")
async def get_active_queue():
    """
    Retrieve active queue entries from database for Staff Portal initialization & reconnect.
    """
    entries = bank_db.list_active_queue_entries()
    return {
        "status": "ok",
        "count": len(entries),
        "queue": entries,
...
async def teller_call(token_id: str, body: TellerCallBody = TellerCallBody()):
...
async def teller_complete(token_id: str):
    ...
    completed_at = bank_db.utc_now_iso()
    bank_db.update_transaction_status(token_id, "COMPLETED")
    bank_db.update_queue_entry_status(token_id, "COMPLETED", completed_at=completed_at)

    # Idempotently ensure balance is applied (if not already applied)
    eff_cust = ctx.customer_id or "cust_sujith"
    if ctx.intent and ctx.entities and ctx.entities.get("amount"):
        bank_db.apply_transaction_balance(
            token_id=token_id,
...
async def teller_expire(token_id: str):
    ...
    bank_db.update_transaction_status(token_id, "EXPIRED")
    bank_db.update_queue_entry_status(token_id, "EXPIRED")
